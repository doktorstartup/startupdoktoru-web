...
// Basılı kitabın tanıtımı + e-kitapla farkını anlatan karşılaştırma.
// Kitap yoksa ya da İngilizce taraftaysak bölüm hiç render edilmez.
  ...
  return (
    <section id="kitap" className="py-20 md:py-32 bg-black/40 border-y border-border/10">
      <div className="max-w-7xl mx-auto px-6 sm:px-8">
        ...
        {/* E-kitapla farkı — karışıklığı burada bitiriyoruz */}
        <div className="mt-20 pt-16 border-t border-border/20">
          <div className="max-w-2xl mx-auto text-center mb-12">
            <h3 className="text-2xl sm:text-3xl font-extrabold tracking-tight">{M.karsilastirmaBaslik}</h3>
            <p className="text-muted-foreground mt-3">{M.karsilastirmaLead}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            ...
            <div className="glass-panel p-8 rounded-2xl border border-border/60 flex flex-col">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-bold text-primary uppercase tracking-widest">{M.ebookEtiket}</span>
              </div>
              <h4 className="text-xl font-bold mb-3">{M.ebookBaslik}</h4>
              <p className="text-sm text-muted-foreground leading-relaxed mb-5">{M.ebookOzet}</p>
              <ul className="space-y-2 mb-6 flex-1">
                {M.ebookOzellikler.map((o) => (
            ...
            {/* Basılı kitap */}
            <div className="glass-panel p-8 rounded-2xl border border-accent/30 flex flex-col">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs font-bold text-accent uppercase tracking-widest">{M.kitapEtiket}</span>
              </div>
              <h4 className="text-xl font-bold mb-3">{KITAP.baslik}</h4>
              <p className="text-sm text-muted-foreground leading-relaxed mb-5">{KITAP.ozet}</p>
              <ul className="space-y-2 mb-6 flex-1">
                {M.kitapOzellikler.map((o) => (
                  <li key={o} className="flex items-start gap-2 text-xs text-foreground/85">
              ...
              {KITAP.fiyat && (
                <div className="flex items-baseline gap-2 mb-4 font-mono">
                  <span className="text-2xl font-extrabold text-accent">{KITAP.fiyat}</span>
                </div>
              )}
              <a href="#kitap" className="btn btn-secondary w-full">
                {cikti ? M.satinAl.replace(":", "") : M.durumYakinda}
              </a>
            </div>
          </div>
        </div>
